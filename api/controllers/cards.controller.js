...
exports.createOne = async (req,res,user) => {
  ...
  const data=req.body
  let query = 'INSERT INTO user_card ( `rnd_id`,`full_name`, `email`, `phone_number`, `fonction` , `societe`, `website`, `theme`, `photo`, `youtube`, `linkedin`, `instagram`, `facebook`)'+
                                            ' VALUES ("000000",?, ?, ?,?, ?, ?,?, ?, ?,? ,? ,?)';
  const values = [data.full_name, data.email, data.phone_number, data.fonction, data.societe, data.website, data.theme,image.filename,data.youtube, data.linkedin, data.instagram,data.facebook];
...
console.log("object")
  const data=req.body
  const query = 'UPDATE user_card SET `full_name`=?, `email`=?, `phone_number`=?, `fonction`=?, `societe`=?, `website`=?, `theme`=?, `photo`=?, `youtube`=?, `linkedin`=?, `instagram`=?, `facebook`=? where id=?'
  const values = [data.full_name, data.email, data.phone_number,data.fonction, data.societe, data.website, data.theme, data.photo,data.youtube, data.linkedin, data.instagram,data.facebook,req.params.id_card];
